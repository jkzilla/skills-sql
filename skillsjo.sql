...
SELECT Models.name, brand_name, headquarters FROM Models LEFT JOIN Brands ON Models.id = Brands.id;
...
SELECT Models.name FROM Models ORDER BY Models.name OFFSET 14 LIMIT 11;
...
SELECT brand_name, models.name, founded FROM Models LEFT JOIN Brands ON brand_name = brands.name
...
Left joins include the whole dataset from the primary table in the FROM statement
as well as selecting data rows that are in the secondary table that match the primary row called data.
...
SELECT brands.name,
    founded 
FROM Brands 
LEFT JOIN Models ON Brands.name = Models.brand_name WHERE Models.brand_name
IS NULL;
